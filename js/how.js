document.addEventListener("DOMContentLoaded", function () {
 ...
 function drawUnderline(element, progress = 1) {
   const containerRect = document.getElementById('how-text').getBoundingClientRect();
   const canvas = document.querySelector('.rough-underline-canvas');
   if (!canvas) return;
   const rc = rough.canvas(canvas);
 
   // Extract color from the style attribute
   const color = element.style.borderColor || '#ffffff'; // Default to white if color is not specified
